(function() {
	...
	es.ui.QueryFilter = acx.ui.Widget.extend({
		...
		_indexSelector_template: function() {
            var indices = [];
            var data = [];
            for(name in this.metadata.indices) {
                indices.push({"name": name, "data": this.metadata.indices[name]});
            }
            indices.sort(function(a, b){
                return a.name < b.name;
            });
            for(i in indices) {
                data.push({ tag: "DIV", cls: "booble queryFilter-index", text: indices[i].name });
            }
			return { tag: "DIV", cls: "section queryFilter-indices", children: [
				{ tag: "HEADER", text: acx.text("QueryFilter-Header-Indices") },
				{ tag: "DIV", onClick: this._selectIndex_handler, children: data}
			] };
		},
...
//		path: "",		 		// (required) path to request
	...
	es.ElasticSearchHead = acx.ui.Widget.extend({
		defaults: {
			base_uri: localStorage["base_uri"] || "http://elastic01.int.ilook.ru:9200/"   // the default ElasticSearch host
		},
		init: function(parent) {
			this._super();
	...
	es.AbstractQuery = acx.ui.Widget.extend({
		defaults: {
            base_uri: "http://elastic01.int.ilook.ru:9200/"
	...
	es.ClusterConnect = es.AbstractQuery.extend({
		...
		_main_template: function() {
            var options = [
                ...
                { value: 'http://br-es-01.kvm.int.ilook.ru:9200/', text: "Brands"},
                { value: 'http://localhost:9200/', text: "Localhost" }
            ];

			return { tag: "SPAN", cls: "es-cluster", children: [
                { tag: "SELECT", cls: "es-header-uri", name: "cron", children: options.map(function(op) {
